...
int Sbox_inv[16][16] = {{0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb},
...
void sub_bytes_inv(uint32_t y[4])
{
	int i, j;
	uint32_t x[4];
	for (i = 0; i < 4; ++i) x[i] = 0x0;
	for(i = 0; i < 4; i++)
	{
		for (j = 0; j < 4; ++j)
		{
			x[i] |= (s_box_inv((y[i] & (0xff000000 >> 8*j) ) >> 8*(3-j) ) ) <<  8*(3-j);
		}
	}
	for (i = 0; i < 4; ++i) y[i] = x[i];
}
// We make the implementation of Shift-Rows.
void shift_rows_inv(uint32_t x[4])
{
	int i;
...
void mix_columns_inv(uint32_t x[4])
{
	int i, j;
	...
	for (i = 0; i < 4; ++i)
	{
		...
		y[0] |= (dot(x1, 0x0e) ^ dot(x2, 0x0b) ^ dot(x3, 0x0d) ^ dot(x4, 0x09) ) << 8*(3-i);
		y[1] |= (dot(x1, 0x09) ^ dot(x2, 0x0e) ^ dot(x3, 0x0b) ^ dot(x4, 0x0d) ) << 8*(3-i);
		y[2] |= (dot(x1, 0x0d) ^ dot(x2, 0x09) ^ dot(x3, 0x0e) ^ dot(x4, 0x0b) ) << 8*(3-i);
		y[3] |= (dot(x1, 0x0b) ^ dot(x2, 0x0d) ^ dot(x3, 0x09) ^ dot(x4, 0x0e) ) << 8*(3-i);
	}
	for (i = 0; i < 4; ++i) x[i] = y[i];
}
// We make the implentation of s-box(x).
int s_box_inv(uint32_t x)
{
	return Sbox_inv[(0xf0 & x) >> 4][(0x0f & x)];
}
...
int main()
{
	...
	printf("\nAES-%d implementation -> decryption\n", nk*32);
    int nr = 4*( (nk + 6) + 1);
    uint32_t words[4*( (nk + 6)+ 1)], keyS[nk];
    fclose ( leer );
    ////////
	leer = fopen("initial_text_cipher.txt","r");
	...
	for(j = ((nk + 6) - 1); j > 0; j--)
	{
		...
		shift_rows_inv(text_initial);
		
		sub_bytes_inv(text_initial);
		initial(text_initial);
		
		add_round_key(text_initial, words, j);
		
		initial(text_initial);
		mix_columns_inv(text_initial);
		initial(text_initial);
	...
	FILE *f = fopen("initial_text.txt", "w");
	if (f == NULL)
	{
